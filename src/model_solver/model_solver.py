...
# The simulation code generated by ModelSolver is stored as instances of the three
# classes below rather than as local functions. Pickle stores classes by reference and
# is unable to store local functions, so closures here would make models unpicklable.
class _ConstantDefFun:
    """Evaluates a definition whose right hand side contains no variables."""

    def __init__(self, rhs_str: str) -> None:
        """Stores the right hand side of the definition."""
        self._rhs_str = rhs_str

    def __call__(self, args: Sequence[Any]) -> NDArray[Any]:
        """Returns the value of the right hand side, ignoring the arguments."""
        return np.array([eval(self._rhs_str)])


class _DefFun:
    """Evaluates a definition given the values of its predetermined variables."""

    def __init__(self, def_fun_lam: Callable[[Sequence[Any]], Any]) -> None:
        """Stores the lambdified right hand side of the definition."""
        self._def_fun_lam = def_fun_lam

    def __call__(self, args: Sequence[Any]) -> NDArray[Any]:
        """Returns the value of the definition for the given arguments."""
        return np.array([self._def_fun_lam(args)], dtype=np.float64)


class _BlockFun:
    """Evaluates an objective function or a Jacobian matrix for a block."""

    def __init__(self, lam: Callable[..., Any]) -> None:
    ...
    def _gen_def_or_obj_fun_and_jac(
        ...
        for eqn in eqns:
            ...
            if len(eqns) == 1 and endo_var == "".join(lhs) and endo_var not in rhs:
                if len(pred_vars) == 0:
                    return _ConstantDefFun(rhs_str), None, None
                try:
                ...
                return _DefFun(def_fun_lam), None, None
        ...
        return None, _BlockFun(obj_fun_lam), _BlockFun(jac_lam)
